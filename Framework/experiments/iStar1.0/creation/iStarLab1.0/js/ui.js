...
ui.defineInteractions = function () {
    ...
    istar.paper.on('cell:pointerup', function (cellView, evt, x, y) {
        ...
        if (ui.currentStateIsAddNode()) {
            ...
            if (cellView.model.isKindOfActor()) {
                if (ui.currentAddingElement.match(/IsALink|IsPartOfLink|OccupiesLink|CoversLink|PlaysLink|INSLink/)) {
                    if (ui.isLinkSourceUndefined()) {
                        ...
                        if (isValid.isValid) {
                            // newLink = istar.addDecompositionLink(ui.linkSource.model, ui.linkTarget.model);
                ...
                if (ui.currentAddingElement.match(/DecompositionLink|MeansEndsLink|ContributionLink|DependencyLink|make|help|hurt|break|some+|some-|unknown|and|or/)) {
                    if (ui.isLinkSourceUndefined()) {
                        ...
                        if (ui.currentAddingElement.match(/DecompositionLink|MeansEndsLink|ContributionLink|make|help|hurt|break|some+|some-|unknown|and|or/)) {
                            var newLink = null;
                            var prettyLinkName = '';
                            if (ui.currentAddingElement === 'DecompositionLink') {
                                isValid = istar.metamodel.nodeLinks[ui.currentAddingElement].isValid(ui.linkSource.model, ui.linkTarget.model);
                                if (isValid.isValid) {
                                    newLink = istar.addDecompositionLink(ui.linkSource.model, ui.linkTarget.model);
                                }
                            }
                            else if (ui.currentAddingElement === 'MeansEndsLink') {
                                isValid = istar.metamodel.nodeLinks[ui.currentAddingElement].isValid(ui.linkSource.model, ui.linkTarget.model);
                                if (isValid.isValid) {
                                    newLink = istar.addMeansEndsLink(ui.linkSource.model, ui.linkTarget.model);
                                }
                            }
                            else if (ui.currentAddingElement.match(/make|help|hurt|break|some+|some-|unknown|and|or/i)) {
...
ui.addElementOnPaper = function (options) {
    try {
        ...
        newActor.prop('customProperties/Description', 'Enter a description here');
...
ui.addElementOnActor = function (cellView, options) {
    try {
        ...
        element.prop('customProperties/Description', 'Enter a description here');
...
function addDependency(source, dependencyType, target) {
    ...
    var text = 'Dependum';
    if (dependencyType === 'SoftgoalDependencyLink') {
        node = istar.addSoftgoal(text, position);
    }
    ...
    node.prop('customProperties/Description', 'Enter a description here');
...
function changeCustomPropertyValue(model, propertyName, propertyValue) {
    if (propertyValue) {
        ...
        propertyValue = 'Enter a description here';
    }
